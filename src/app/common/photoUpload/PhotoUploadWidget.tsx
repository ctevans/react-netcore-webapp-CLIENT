...
const PhotoUploadWidget: React.FC<IProps> = ({ loading, uploadPhoto }) => {
    const [files, setFiles] = useState<any[]>([]); //INTENTIONALLY BYPASS TYPE CHECKING.
    const [image, setImage] = useState<Blob | null>(null);

    //Cleanup to prevent memory leak from image.
    //Removes as soon as image is put into the URL.
    useEffect(() => {
        return () => {
                    ...
                    <Header sub color='teal' content='Step 3 - Preview & Upload' />
                    {
                        ...
                        <Fragment>
                            ...
                            <Button.Group widths={2}>
                                ...
                                <Button positive icon='close' disabled={loading} onClick={() => setFiles([])} />
                            </Button.Group>
                        </Fragment>
                    }
                </Grid.Column>
